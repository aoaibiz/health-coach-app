...
export function MealCard({
  ...
  const hasCardImage = Boolean(imagePlan.syncedImageSrc || imagePlan.legacyGeneratedImageId);
  // Manual generate/regenerate is offered whenever the meal has a prompt — even
  // when an image already exists — so a wrong/stale image can be replaced (Ao
  // 2026-06-28). Auto-generation (needsAutoGeneratedImage) is unchanged, so we
  // never auto-regenerate an image that is already present.
  const canGenerateImage = !!onGenerateImage && Boolean(imagePlan.promptText);
  return (
    <div
        ...
        <div className="mb-1.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            {canGenerateImage && (
              ...
              >
                {generatingImage ? "生成中" : hasCardImage ? "再生成" : "画像生成"}
              </button>
            )}
            <button
              type="button"
